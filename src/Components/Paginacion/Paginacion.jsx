      ...
      <div className="hidden sm:flex-1 sm:flex sm:items-end sm:justify-end">
        <div>
          <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
            {actual > 1 && (
              <Link to={`/store/pag/${actual-1}`} className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
            ...
            {actual > 3 && (<Link to={`/store/pag/${actual-1}`} className="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 hidden md:inline-flex relative items-center px-4 py-2 border text-sm font-medium"> {parseInt(actual)-1} </Link>)}
            {actual > 3 && actual < paginasTotales && (<Link to={`/store/pag/${actual}`} className="bg-indigo-50 text-indigo-600 hidden md:inline-flex relative items-center px-4 py-2 border text-sm font-medium"> {parseInt(actual)} </Link>)}
            {actual > 3 && actual < paginasTotales - 1 && (<Link to={`/store/pag/${actual+1}`} className="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 hidden md:inline-flex relative items-center px-4 py-2 border text-sm font-medium"> {parseInt(actual)+1} </Link>)}
            ...
            <Link to={`/store/pag/${paginasTotales}`} className={`${(actual == paginasTotales)?'bg-indigo-50 text-indigo-600':'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'} relative inline-flex items-center px-4 py-2 border text-sm font-medium`}> {paginasTotales} </Link>
            {actual < paginasTotales && (
              <Link to={`/store/pag/${actual+1}`} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
